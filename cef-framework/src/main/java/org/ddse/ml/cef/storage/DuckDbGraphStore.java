...
/**
 * DuckDB JDBC implementation of GraphStore with dual persistence.
 * Uses DuckDbNodeRepository and DuckDbEdgeRepository (JDBC with reactive
 * wrappers).
 * Coordinates between database persistence (JDBC DuckDB) and in-memory graph
 * (JGraphT).
 * 
...
public class DuckDbGraphStore implements GraphStore {
    ...
    private final DuckDbNodeRepository nodeRepository;
    private final DuckDbEdgeRepository edgeRepository;
    private final InMemoryKnowledgeGraph inMemoryGraph;

    public DuckDbGraphStore(DuckDbNodeRepository nodeRepository,
            DuckDbEdgeRepository edgeRepository,
    ...
    public Mono<Void> initialize(List<RelationType> relationTypes) {
        log.info("Initializing DuckDB dual persistence graph store");
        return Mono.empty();
    }
    ...
    public Mono<Edge> getEdge(UUID edgeId) {
        // Read from database (in-memory graph doesn't index edges by ID directly)
     ...
     * Called on application startup via ApplicationRunner.
     */
    public Mono<Void> loadFromDatabase() {
        log.info("Loading graph from DuckDB database into memory...");
        long start = System.currentTimeMillis();

        return nodeRepository.findAll()
                .collectList()
                .zipWith(edgeRepository.findAll().collectList())
                .doOnNext(tuple -> {
