...
**Status:** Accepted  
**Date:** 2024-11-24 (Updated: 2024-11-27)  
...
**UPDATE 2024-11-27:** Enhanced hybrid retrieval to better support **multi-hop graph reasoning** through improved path extraction, constraint filtering during traversal, and better utilization of relation type hints.

The framework's primary purpose is to **enable reasoning through relationship paths** and provide **context keywords for Vector Similarity Search (VSS)**, with enhanced capabilities for multi-hop queries.
...
### Hybrid Retrieval Enhancements (2024-11-27)

**Improved Multi-Hop Support:**
- Better utilization of relation type hints during traversal
- Path-specific extraction instead of generic k-hop subgraph
- Constraint filtering at each traversal step
- Support for longer paths (3-8 hops) with filtering

**Key Improvements:**
1. **Relation-Guided Traversal** - Use ALL relation hints from graphQuery, not just first one
2. **Constraint Filtering** - Apply property filters during traversal, not after
3. **Path Extraction** - Extract specific paths matching hints, not entire neighborhood
4. **Multi-Path Queries** - Support multiple independent path queries with intersection/union
5. **Better Context Assembly** - Prioritize nodes along specified relation paths
...
The existing hybrid approach (vector-first entry point resolution + graph traversal + semantic search) is enhanced with structured path patterns and constraint filtering. **No new interfaces** - these are additions to the existing GraphQuery and KnowledgeRetriever system.

### 6. GraphPattern - Structured Path Specification

```java
/**
 * NEW: Structured graph pattern for the hybrid retrieval system.
 * Used within GraphQuery to specify multi-hop paths with constraints.
 * Processed by existing KnowledgeRetriever during hybrid search.
 */
public record GraphPattern(
    String patternId,                     // Unique identifier for this pattern
    List<TraversalStep> steps,            // Step-by-step traversal specification
    List<Constraint> constraints,         // Constraints to apply during traversal
    String description                    // Human-readable description
) {
    /**
     * Example: Multi-hop contraindication discovery (3-hop)
     * Pattern: Patient→Condition→Patient→Medication
     */
    public static GraphPattern multiHopContraindication() {
        return new GraphPattern(
            "multi_hop_contraindication",
            List.of(
            ...
            List.of(
                new Constraint(ConstraintType.PROPERTY_IN, "Medication", "name", 
                    List.of("Methotrexate", "Prednisone"), 2)
            ),
            "Find patients with conditions diagnosed in other patients taking specific medications"
        );
    }
}
```

### 7. TraversalStep - Single Hop Specification

```java
/**
 * Single step in graph traversal pattern.
 * Specifies: start node type → relation → target node type
 */
public record TraversalStep(
    String sourceLabel,                   // Starting node label (null = use previous step result)
    String relationType,                  // Edge type to traverse
    String targetLabel,                   // Target node label to reach
    int stepIndex,                        // Position in traversal sequence (0-based)
    Direction direction                   // OUTGOING, INCOMING, or BOTH (default: OUTGOING)
) {
...
### 8. Constraint - Filtering During Traversal

```java
/**
 * Constraint to apply during graph traversal.
 * Enables filtering at specific steps (not just final results).
 */
public record Constraint(
    ConstraintType type,                  // Type of constraint
    String nodeLabel,                     // Node label to apply constraint to
    String propertyPath,                  // Property path (e.g., "name", "properties.category")
    Object value,                         // Expected value(s)
    int atStep                            // Apply at this step index (-1 = all steps)
) {
    /**
     * Convenience constructors for common constraints
     */
...
### 9. QueryCombinator - Multi-Pattern Queries

```java
/**
 * Combine multiple graph patterns with logical operators.
 * Enables complex queries requiring multiple independent paths.
 */
public record QueryCombinator(
    CombinatorType type,                  // How to combine results
    List<GraphPattern> patterns,          // Patterns to execute
    String description                    // Human-readable description
) {
    /**
     * Example: Polypharmacy risk with 5 independent constraints (intersection)
     */
    public static QueryCombinator polypharmacyRisk() {
        return new QueryCombinator(
            CombinatorType.INTERSECTION,
            List.of(
                // Pattern 1: Has RA
                new GraphPattern("has_ra", 
                    List.of(new TraversalStep("Patient", "HAS_CONDITION", "Condition", 0)),
                    List.of(Constraint.propertyEquals("Condition", "name", "Rheumatoid Arthritis", 0)),
                    "Patients with RA"
                ),
                // Pattern 2: Takes Albuterol
                new GraphPattern("takes_albuterol",
                    List.of(new TraversalStep("Patient", "TAKES_MEDICATION", "Medication", 0)),
                    List.of(Constraint.propertyEquals("Medication", "name", "Albuterol", 0)),
                    "Patients taking Albuterol"
                ),
                // Pattern 3: Has Diabetes
                new GraphPattern("has_diabetes",
                    List.of(new TraversalStep("Patient", "HAS_CONDITION", "Condition", 0)),
                    List.of(Constraint.propertyEquals("Condition", "name", "Diabetes", 0)),
                    "Patients with Diabetes"
                ),
                // Pattern 4: Shares doctor with high-risk patient
                new GraphPattern("shared_doctor",
                    List.of(
                        new TraversalStep("Patient", "TREATED_BY", "Doctor", 0),
                        new TraversalStep("Doctor", "TREATS", "Patient", 1)
                    ),
                    List.of(Constraint.propertyEquals("Patient", "riskLevel", "high", 1)),
                    "Shares doctor with high-risk patient"
                ),
                // Pattern 5: Has abnormal lab value
                new GraphPattern("abnormal_labs",
                    List.of(new TraversalStep("Patient", "HAS_LAB_RESULT", "LabResult", 0)),
                    List.of(Constraint.greaterThan("LabResult", "value", 120.0, 0)),
                    "Has abnormal lab results"
                )
            ),
            "Find patients matching ALL 5 risk criteria"
        );
    }
}

/**
 * Combinator types for multi-pattern queries
 */
public enum CombinatorType {
    INTERSECTION,         // AND - all patterns must match (set intersection)
    UNION,                // OR - any pattern matches (set union)
    SEQUENTIAL            // Execute patterns in order, feed results forward
}
```

### 10. MatchedPath - Query Execution Result
...
public record MatchedPath(
    String patternId,                     // Which pattern matched
    List<UUID> nodeIds,                   // Nodes in the path (ordered)
    List<String> relationTypes,           // Edges traversed (ordered)
    Map<String, Object> pathProperties,   // Aggregated properties along path
    double score,                         // Path relevance score
    String explanation                    // Why this path matched
) {
    /**
     * Get node at specific step
     */
    ...
    /**
     * Get full path as string for debugging
     */
...
### 11. RankingStrategy - Path Scoring

```java
/**
 * Strategy for ranking matched paths.
 * Determines which paths are most relevant to the query.
 */
public enum RankingStrategy {
    PATH_LENGTH,          // Prefer shorter paths
    EDGE_WEIGHT,          // Sum edge weights (if present)
    NODE_CENTRALITY,      // Prefer paths through high-centrality nodes
    SEMANTIC_SCORE,       // Combine with vector similarity scores
...
public interface KnowledgeRetriever {
     ...
     * 
     * ENHANCED Hybrid Search with Pattern Support:
     * 1. Resolve entry points from graphQuery using Vector Search
     * 2. IF patterns provided:
     *    a. Execute step-by-step traversal following TraversalSteps
     *    b. Apply Constraints at each hop to filter nodes
     *    c. Return MatchedPath objects in reasoningContext
     * 3. ELSE (no patterns):
     *    a. Extract k-hop subgraph (legacy behavior)
     * 4. Enhance query with context keywords
     * 5. Perform Hybrid Search (graph-constrained VSS + vector scores)
     * 6. Combine rankings (path scores + semantic scores)
     * 
     * @param query Structured graph query with optional patterns
     * @return SearchResult with chunks, context (including matched paths), strategy used
     */
    @MCPTool(
        name = "retrieve_knowledge",
        description = "Retrieve relevant knowledge. MUST use Graph Schema to construct graphQuery with patterns for multi-hop queries."
    )
 ...
 * 
 * HYBRID APPROACH:
 * 1. Vector-first entry point resolution (targets OR first step of pattern)
 * 2. Path-specific traversal (patterns with constraints)
 * 3. Semantic search on matched nodes
 * 4. Combined ranking (graph + vector scores)
 */
public record GraphQuery(
    String naturalLanguageQuery,          // Required: Original user question
    
    // Vector-first entry points (existing)
    List<ResolutionTarget> targets,       // Optional: Vector search entry points
    TraversalHint traversal,              // Optional: Simple traversal hints
    
    // ENHANCED: Structured patterns (NEW - processed by existing KnowledgeRetriever)
    List<GraphPattern> patterns,          // Optional: Multi-hop path patterns with constraints
    QueryCombinator combinator,           // Optional: Combine multiple patterns (AND/OR/SEQUENTIAL)
    RankingStrategy rankingStrategy,      // Optional: Path scoring (default: HYBRID)
    ...
    int maxPaths                          // Max paths to return (default: 100)
) {
    /**
...
public record SearchResult(
    ...
    ReasoningContext reasoningContext,    // ENHANCED: now includes matched paths
...
/**
 * ENHANCED: Reasoning context now includes structured matched paths
...
public record ReasoningContext(
    ...
    List<MatchedPath> matchedPaths,       // ENHANCED: Specific paths matching patterns
...
public enum SearchStrategy {
    HYBRID,           // ENHANCED: Graph patterns + constraints + vector search
    VECTOR,           // Pure vector similarity search
    BM25              // Pure full-text search
}
```
...
cef:
  ...
  search:
    ...
    # Enhanced Hybrid settings (2024-11-27)
    patterns:
      enabled: true                      # Enable pattern-based traversal in hybrid search
      max-paths: 100                     # Max paths to match per pattern
      default-ranking: HYBRID            # PATH_LENGTH | EDGE_WEIGHT | NODE_CENTRALITY | SEMANTIC_SCORE | HYBRID
      constraint-evaluation: strict      # strict | lenient (stop on first failure vs collect all)
      use-all-relation-hints: true       # Use ALL relation hints, not just first one
